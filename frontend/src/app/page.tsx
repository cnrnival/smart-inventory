...
export default function DashboardPage() {
  ...
  // ✅ Controle de Hidratação para o Vercel/Next.js não dar erro nos gráficos
  ...
  // Lógica de cálculo FEFO (Mantida exatamente a sua)
  ...
  // ✅ Proteção de build: Renderiza apenas no cliente
    ...
    <div className="min-h-screen p-4 md:p-8 text-white bg-[#262626]">
      <div className="mx-auto max-w-6xl space-y-8">
        <header className="flex flex-col items-start justify-between gap-4 rounded-xl border border-gray-700 bg-[#323232] p-6 shadow-sm sm:flex-row sm:items-center">
          ...
          <button onClick={logout} className="inline-flex items-center gap-2 rounded-lg bg-red-500/10 px-4 py-2 text-sm font-medium text-red-500 hover:bg-red-500/20">
            <LogOut size={16} /> Sair
          </button>
        </header>
        ...
        <div className="rounded-xl border border-gray-700 bg-[#323232] overflow-hidden">
          ...
          <table className="w-full text-left">
            <thead className="bg-[#262626] text-gray-400 text-sm">
              <tr><th className="px-6 py-4">Produto</th><th className="px-6 py-4">Validade</th><th className="px-6 py-4">Status</th></tr>
            </thead>
            <tbody className="divide-y divide-gray-700">
              {products.slice(0, 5).map((product) => (
                <tr key={product.id} className="hover:bg-white/5 transition-colors">
                  ...
                  <td className="px-6 py-4"><span className="px-2 py-1 rounded-full text-xs bg-red-500/20 text-red-500 border border-red-500/50">Monitorado</span></td>
                </tr>
              ))}
            </tbody>
          </table>
