...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const response = await axios_api.get(`/users?email=${email.trim()}&password=${password.trim()}`);
    ...
    <div className="flex min-h-screen items-center justify-center bg-[#212121] p-4 text-white font-sans">
      <div className="w-full max-w-md rounded-2xl bg-[#333333] p-10 shadow-2xl border border-gray-600">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <button type="submit" disabled={isLoading} className="w-full rounded-lg bg-[#6b9dff] py-4 text-lg font-bold text-white hover:opacity-90 disabled:opacity-50">
